...
const UpcomingCar = () => {
    ...
    return (
        <div className='w-full mt-10 md:mt-[200px]'>
            <h1 className='text-2xl font-bold mb-5 flex gap-2 justify-center items-center'> <span className='text-green-600'><BiSkipNextCircle /></span> Upcoming Car </h1>
            <hr />
            <div className='grid grid-cols-1 md:grid-cols-5 gap-5 mt-10'>
                <div className='col-span-2 '>
                    <div>
                        <h1 className='text-lg font-bold mb-5 flex gap-2 justify-center items-center'> <span className='text-green-600'><BiSolidAddToQueue /></span> Add Upcoming Car </h1>
                    </div>
                ...
                <div className='col-span-3'>
                    <h1 className='text-lg font-bold mb-5 flex gap-2 justify-center items-center'> <span className='text-green-600'><TbPlayerTrackNext /></span>Upcoming Car List</h1>
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                        { loading? <Loader/> :
                            [...carData].reverse().map((data) => (
                                <div key={data?._id} className='p-5 border border-green-300 rounded-md hover:shadow-green-400 hover:shadow-md'>
                                    <div className='flex justify-center'>
                                        <img src={data.images[0]} className='rounded ' alt="" />
                                    </div>
                                    <h2 className='font-bold'>{data.basicData.carName}</h2>
                                    <p className='font-semibold flex items-center'>{data.basicData.price} <span className='text-green-600'><IoLogoUsd /></span> </p>
                                </div>
                            ))
