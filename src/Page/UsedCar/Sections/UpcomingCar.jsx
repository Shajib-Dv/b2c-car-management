      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 px-10">
        {carData &&
          ...
          carData.slice(0, limit ? +limit : carData.length).map((car) => (
            <div
              className="shadow border p-3 rounded-lg flex flex-col justify-between gap-2 group"
              key={car?._id}
            >
              ...
              <div>
                <h1 className="font-bold">
                  {car?.basicData?.carName || "unknown"}
                </h1>
                <p>
                  Rs {car?.basicData?.price} - {+car?.basicData?.price + -1500}{" "}
                  Lakh*
                </p>
